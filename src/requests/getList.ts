...
export const getList = (query: string):Promise<any> => new Promise((resolve, reject) => {
    ...
    axios(config)
        .then((response) => {
            resolve(response.data["{http://www.ebayclassifiedsgroup.com/schema/ad/v1}ads"].value.ad);
        })
        .catch((error) => {
            console.log(error);
            reject(error)
